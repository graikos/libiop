    ...
    num_variables_(num_variables)
{
    ...
    systematic_domain_size = libff_liop::round_to_next_power_of_2(systematic_domain_size);
    this->systematic_domain_dim_ = libff_liop::log2(systematic_domain_size);
...
void ligero_iop_parameters<FieldT>::print() const
{
    ...
    libff_liop::print_indent(); printf("* target security parameter = %zu\n", this->security_parameter_);
    libff_liop::print_indent(); printf("* achieved security parameter = %.1Lf\n", this->achieved_soundness());
    libff_liop::print_indent(); printf("* encoded ligero interactions = %lu\n",
        this->encoded_ligero_params_.num_interaction_phase_repetitions_);
    libff_liop::print_indent(); printf("* encoded ligero queries = %lu\n",
        this->encoded_ligero_params_.num_query_phase_repetitions_);
    libff_liop::print_indent(); printf("* encoded ligero absolute proximity parameter = %lu\n",
        this->absolute_encoded_ligero_proximity_parameter_);
    libff_liop::print_indent(); printf("* encoded ligero fractional proximity parameter = %Lf\n",
        this->fractional_encoded_ligero_proximity_parameter_);
    libff_liop::print_indent(); printf("* RS extra dimensions = %zu\n", this->RS_extra_dimensions_);
    libff_liop::print_indent(); printf("* systematic domain dim = %zu\n", this->systematic_domain_dim_);
    libff_liop::print_indent(); printf("* codeword domain dim = %zu\n", this->codeword_domain_dim_);
    libff_liop::print_indent(); printf("* num oracles for input = %zu\n", this->num_oracles_input_);
    libff_liop::print_indent(); printf("* num oracle vectors = %zu\n", this->num_oracle_vectors_);
    libff_liop::print_indent(); printf("* make zk = %s\n", (this->make_zk_ ? "true" : "false"));
...
{
    libff_liop::enter_block("Check Interleaved R1CS verifier predicate");
    bool decision = this->protocol_->verifier_predicate(primary_input);
    libff_liop::leave_block("Check Interleaved R1CS verifier predicate");

    libff_liop::enter_block("Check LDT verifier predicate");
    decision &= this->LDT_reducer_->verifier_predicate();
    libff_liop::leave_block("Check LDT verifier predicate");
