    ...
    const FieldT x_i)
{
    ...
    const FieldT h_inc_to_coset_inv_plus_one =
        libff_liop::power(h_inc, coset_size).inverse() * h_inc;
    ...
    const FieldT x_to_order_coset = libff_liop::power(x_i, coset_size);
    /* xg^{-k} */
    ...
    const FieldT first_h_to_coset_inv_plus_one = libff_liop::power(cur_h, coset_size).inverse() * cur_h;
    ...
    const FieldT x_i)
{
    ...
    const FieldT vp_x = libff_liop::power(x_i, coset_size) -
        libff_liop::power(h, coset_size);
    const bool x_in_domain = vp_x == FieldT::zero();
    const FieldT c = vp_x * (FieldT(m) * libff_liop::power(h, coset_size - 1)).inverse();
