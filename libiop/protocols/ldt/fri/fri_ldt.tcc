...
FRI_protocol_parameters<FieldT>::FRI_protocol_parameters(const size_t interactive_soundness_bits,
    ...
    field_type_(libff_liop::get_field_type<FieldT>(FieldT::zero())),
    localization_parameters_(localization_parameter_array)
{
    ...
    const long double field_size_bits = (long double)(libff_liop::soundness_log_of_field_size_helper<FieldT>(FieldT::zero()));
...
template<typename FieldT>
libff_liop::field_type FRI_protocol_parameters<FieldT>::get_field_type() const
{
...
void FRI_protocol_parameters<FieldT>::print() const
{
    ...
    {
        libff_liop::print_indent(); printf("===WARNING=== FRI security parameter was overridden\n");
    }
    libff_liop::print_indent(); printf("* soundness type = %s\n", FRI_soundness_type_to_string(this->soundness_type_));
    libff_liop::print_indent(); printf("* target interactive soundness error (bits) = %zu\n", this->target_interactive_security_parameter_);
    libff_liop::print_indent(); printf("* achieved interactive soundness error (bits) = %.1Lf\n", this->achieved_interactive_soundness());
    libff_liop::print_indent(); printf("* target query soundness error (bits) = %zu\n", this->target_query_security_parameter_);
    libff_liop::print_indent(); printf("* achieved query soundness error (bits) = %.1Lf\n", this->achieved_query_soundness());
    libff_liop::print_indent(); printf("* codeword domain dim = %zu\n", this->codeword_domain_dim_);
    libff_liop::print_indent(); printf("* effective proximity parameter = %Lf\n", this->effective_proximity_parameter_);
    libff_liop::print_indent(); printf("* number of interactive repetitions = %zu\n", this->num_interactive_repetitions_);
    libff_liop::print_indent(); printf("* number of query repetitions = %zu\n", this->num_query_repetitions_);
    libff_liop::print_indent(); printf("* localization parameter array = ");
    libff_liop::print_vector<size_t>(this->localization_parameters_);
    libff_liop::print_indent(); printf("* number of reductions = %zu\n", this->localization_parameters_.size());
}
...
void FRI_protocol<FieldT>::compute_domains()
{
    ...
    if (this->params_.get_field_type() == libff_liop::multiplicative_field_type)
    {
    ...
    else if (this->params_.get_field_type() == libff_liop::additive_field_type)
    {
        for (std::size_t i = 0; i < this->num_reductions_; ++i)
        {
...
void FRI_protocol<FieldT>::calculate_and_submit_proof()
{
    ...
    for (std::size_t i = 0; i < this->num_reductions_; ++i)
    {
        ...
        if (i > 0)
        {
            ...
            libff_liop::enter_block("LDT signal prover round done");
            this->IOP_.signal_prover_round_done();
            libff_liop::leave_block("LDT signal prover round done");
        }
        ...
        for (size_t j = 0; j < this->params_.interactive_repetitions(); j++)
        {
            ...
            libff_liop::enter_block("evaluating next FRI codeword");
            for (size_t ldt_index = 0; ldt_index < this->poly_handles_.size(); ldt_index++)
            {
            ...
            libff_liop::leave_block("evaluating next FRI codeword");
        }
    }
...
bool FRI_protocol<FieldT>::verifier_predicate()
{
    ...
    libff_liop::print_indent(); printf("* Number of FRI interactions: %zu\n", this->params_.interactive_repetitions());
    libff_liop::print_indent(); printf("* Number of FRI query sets per interaction: %zu\n",
        this->query_sets_.size() / this->params_.interactive_repetitions());
    libff_liop::print_indent(); printf("* Total number of FRI queries over all query sets (incl. repeated queries): %zu\n", this->num_queries_made_);
    // libff_liop::print_indent(); printf("* Analytical expression: %zu\n", (2 + ((this->num_reductions_ - 1) << this->localization_parameter_)) * this->query_sets_.size());
