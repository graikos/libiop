...
void LDT_instance_reducer_params<FieldT>::print() const
{
    ...
    {
        libff_liop::print_indent(); printf("===WARNING=== LDT instance reducer security parameter was overridden\n");
    }
    ...
    libff_liop::print_indent(); printf("* max tested degree bound = %zu\n", this->max_tested_degree_bound_);
    libff_liop::print_indent(); printf("* max constraint degree bound = %zu\n", this->max_constraint_degree_bound_);
    libff_liop::print_indent(); printf("* absolute proximity parameter = %zu\n", this->absolute_proximity_parameter_);
    libff_liop::print_indent(); printf("* fractional proximity parameter = %Lf\n", this->fractional_proximity_parameter_);
    libff_liop::print_indent(); printf("* num output LDT instances = %zu\n", this->num_output_LDT_instances_);
}
    ...
    reducer_params_(params)
{
    ...
    libff_liop::UNUSED(base_checker::value);
...
{
    libff_liop::enter_block("LDT Reducer: Submit masking polynomial");
    if (this->reducer_params_.make_zk())
    {
        for (size_t i = 0; i < this->reducer_params_.num_output_LDT_instances(); ++i)
        {
    ...
    libff_liop::leave_block("LDT Reducer: Submit masking polynomial");
}

template<typename FieldT, typename multi_LDT_type>
void LDT_instance_reducer<FieldT, multi_LDT_type>::calculate_and_submit_proof()
{
    libff_liop::enter_block("LDT Reducer: Calculate and submit proof");
    for (size_t i = 0; i < this->reducer_params_.num_output_LDT_instances(); ++i)
    {
    ...
    libff_liop::leave_block("LDT Reducer: Calculate and submit proof");
}
