...
{
    if (!libff_liop::is_power_of_2(num_constraints))
    {
        char err_msg [1000];
        const size_t next_power_of_two = 1ull << libff_liop::log2(num_constraints);
    ...
    if (!libff_liop::is_power_of_2(num_variables + 1))
    {
        char err_msg [1000];
        const size_t next_power_of_two = 1ull << libff_liop::log2(num_variables + 1);
    ...
    this->constraint_domain_dim_ = libff_liop::log2(num_constraints);
    this->variable_domain_dim_ = libff_liop::log2(num_variables + 1);
...
void aurora_iop_parameters<FieldT>::print() const
{
    ...
    libff_liop::print_indent(); printf("* constraint domain dim = %zu\n", this->constraint_domain_dim_);
    libff_liop::print_indent(); printf("* variable domain dim = %zu\n", this->variable_domain_dim_);
    libff_liop::print_indent(); printf("* query bound = %zu\n", this->query_bound_);
...
{
    if (!libff_liop::is_power_of_2(this->constraint_system_.num_inputs() + 1))
    {
...
{
    libff_liop::enter_block("Construct R1CS verifier state");
    this->protocol_->construct_verifier_state(primary_input);
    libff_liop::leave_block("Construct R1CS verifier state");
