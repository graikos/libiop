    ...
    constraint_system_(constraint_system)
{
    ...
    if (!libff_liop::is_power_of_2(this->constraint_system_->num_constraints()))
    {
    ...
    const size_t index_domain_dim = libff_liop::log2(max_num_nonzero_entries_per_matrix);
    ...
    this->codeword_domain_dim_ = libff_liop::log2(max_tested_degree) + this->RS_extra_dimensions_;
...
void fractal_iop_parameters<FieldT>::print() const
{
    ...
    libff_liop::print_indent(); printf("* matrix domain dim = %zu\n", this->matrix_domain_.dimension());
    libff_liop::print_indent(); printf("* index domain dim = %zu\n", this->index_domain_.dimension());
...
{
    const size_t input_variable_dim = libff_liop::log2(this->parameters_.constraint_system()->num_inputs());
