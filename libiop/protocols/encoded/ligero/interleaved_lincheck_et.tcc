...
bool interleaved_lincheck_et_protocol<FieldT>::verifier_predicate()
{
    ...
    for (size_t h = 0; h < this->num_interactions_; ++h)
    {
        ...
        libff_liop::enter_block("Lincheck: equality test (polynomial matches randomized vector)");
        ...
        libff_liop::leave_block("Lincheck: equality test (polynomial matches randomized vector)");
        ...
        for (size_t k = 0; k < this->num_queries_; ++k)
        {
            ...
            {
#ifdef DEBUG
                libff_liop::print_indent(); printf("For interactive repetition h = %zu and query position = %zu,\n", h, j);
