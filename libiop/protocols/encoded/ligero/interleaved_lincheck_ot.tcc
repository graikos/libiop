                                                                              ...
                                                                              std::vector<std::vector<FieldT>> random_linear_combinations)
{
    ...
    libff_liop::enter_block("Input consistency");
    ...
    libff_liop::leave_block("Input consistency");
                                                                  ...
                                                                  std::vector<std::vector<FieldT>> lagrange_coefficients)
{
    ...
    for (size_t h = 0; h < this->num_interactions_; ++h)
    {
        ...
        libff_liop::enter_block("Lincheck: equality test (p_0 sums to 0 over systematic domain)");
        ...
        {
#ifdef DEBUG
            libff_liop::print_indent(); printf("For interactive repetition h = %zu\n", h);

            libff_liop::print_indent(); libff_liop::print_indent();
        ...
        libff_liop::leave_block("Lincheck: equality test (p_0 sums to 0 over systematic domain)");
        ...
        if (!using_lagrange)
        {
            ...
            libff_liop::enter_block("Precomputing for consistency tests");
            ...
            libff_liop::leave_block("Precomputing for consistency tests");
        }

        libff_liop::enter_block("Lincheck: querying and performing consistency tests");
        for (size_t k = 0; k < this->num_queries_; ++k)
        {
            const random_query_position_handle this_position_handle = this->query_position_handles_[k];
            const std::size_t j = this->IOP_.obtain_query_position(this_position_handle);
            ...
            {
#ifdef DEBUG
                libff_liop::print_indent(); printf("For interactive repetition h = %zu and query position j = %zu,\n", h, j);
        ...
        libff_liop::leave_block("Lincheck: querying and performing consistency tests");
    }
