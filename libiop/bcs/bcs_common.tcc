...
#include <numeric>
#include <set>
#include <libff_liop/algebra/field_utils/bigint.hpp>
...
std::size_t bcs_transformation_transcript<FieldT, MT_root_hash>::IOP_size_in_bytes() const
{
    const size_t field_size =
        (libff_liop::log_of_field_size_helper<FieldT>(FieldT::zero()) + 7) / 8;
...
std::ostream& serialize_transcript_internal(
    typename libff_liop::enable_if<libff_liop::is_multiplicative<FieldT>::value, FieldT>::type,
    typename libff_liop::enable_if<std::is_same<MT_hash_type, FieldT>::value, FieldT>::type,
    ...
    parameters_(parameters)
{
    ...
    printf("\nBCS parameters\n");
    libff_liop::print_indent(); printf("* digest_len (bytes) = %zu\n", this->digest_len_bytes_);
    libff_liop::print_indent(); printf("* digest_len (bits) = %zu\n", 8 * this->digest_len_bytes_);
    libff_liop::print_indent(); printf("* hash_type = %s\n", bcs_hash_type_names[parameters.hash_enum]);
}
    ...
    const bcs_protocol<FieldT, MT_root_hash> bcs)
{
    ...
    const size_t field_size = (libff_liop::log_of_field_size_helper<FieldT>(FieldT::zero()) + 7) / 8;
