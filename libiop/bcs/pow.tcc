...
void pow_parameters::print() const
{
    ...
    libff_liop::print_indent(); printf("* log of target work amount = %zu\n", this->work_parameter_);
    libff_liop::print_indent(); printf("* assumed cost per hash = %zu\n", this->cost_per_hash_);
    libff_liop::print_indent(); printf("* expected number of hashes = %d\n", 1 << this->pow_bitlen());
}
...
hash_digest_type pow<FieldT, hash_digest_type>::solve_pow_internal(
    const two_to_one_hash_function<hash_digest_type> &node_hasher, 
    const typename libff_liop::enable_if<std::is_same<hash_digest_type, FieldT>::value, hash_digest_type>::type challenge) const
{
...
hash_digest_type pow<FieldT, hash_digest_type>::solve_pow_internal(
    const two_to_one_hash_function<hash_digest_type> &node_hasher, 
    const typename libff_liop::enable_if<std::is_same<hash_digest_type, binary_hash_digest>::value, hash_digest_type>::type challenge) const
{
...
bool pow<FieldT, hash_digest_type>::verify_pow_internal(
    const typename libff_liop::enable_if<std::is_same<hash_digest_type, FieldT>::value, hash_digest_type>::type &hash) const
{
    size_t least_significant_word = libff_liop::get_word_of_field_elem<FieldT>(hash, 0);
...
template<typename FieldT, typename hash_digest_type>
bool pow<FieldT, hash_digest_type>::verify_pow_internal(
    const typename libff_liop::enable_if<std::is_same<hash_digest_type, binary_hash_digest>::value, hash_digest_type>::type &hash) const
{
