...
template<typename hash_type>
size_t get_hash_size(const typename libff_liop::enable_if<!std::is_same<hash_type, binary_hash_digest>::value, hash_type>::type h)
{
    const size_t field_size =
        (libff_liop::log_of_field_size_helper<hash_type>(hash_type::zero()) + 7) / 8;
    return field_size;
}
