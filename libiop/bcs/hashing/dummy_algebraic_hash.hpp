...
class dummy_algebraic_hashchain : public hashchain<FieldT, MT_root_type>
{
    ...
    protected:
        void absorb_internal(const typename libff_liop::enable_if<std::is_same<MT_root_type, binary_hash_digest>::value, MT_root_type>::type new_input);
        void absorb_internal(const typename libff_liop::enable_if<std::is_same<MT_root_type, FieldT>::value, MT_root_type>::type new_input);

        MT_root_type squeeze_root_type_internal(const typename libff_liop::enable_if<std::is_same<MT_root_type, binary_hash_digest>::value, MT_root_type>::type dummy);
        MT_root_type squeeze_root_type_internal(const typename libff_liop::enable_if<std::is_same<MT_root_type, FieldT>::value, MT_root_type>::type dummy);
};
