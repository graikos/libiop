...
FieldT string_to_field_elem(
    typename libff_liop::enable_if<libff_liop::is_multiplicative<FieldT>::value, FieldT>::type dummy_field_elem,
    const zk_salt_type &zk_salt)
{
    libff_liop::bigint<FieldT::num_limbs> num(0ul);
...
template<typename FieldT>
FieldT string_to_field_elem(
    typename libff_liop::enable_if<libff_liop::is_additive<FieldT>::value, FieldT>::type dummy_field_elem,
    const zk_salt_type &zk_salt)
{
...
void algebraic_hashchain<FieldT, MT_root_type>::absorb_internal(
    const typename libff_liop::enable_if<std::is_same<MT_root_type, binary_hash_digest>::value, MT_root_type>::type new_input)
{
...
template<typename FieldT, typename MT_root_type>
void algebraic_hashchain<FieldT, MT_root_type>::absorb_internal(
    const typename libff_liop::enable_if<std::is_same<MT_root_type, FieldT>::value, MT_root_type>::type new_input)
{
    ...
    size_t num_elements, size_t range_of_positions)
{
    ...
    {
        positions.emplace_back(libff_liop::get_word_of_field_elem<FieldT>(squeezed_elems[i], word_index) % range_of_positions);
    }
    return positions;
}
