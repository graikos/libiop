...
{
    this->bcs_params_ = default_bcs_params<FieldT, hash_type>(hash_enum, this->security_parameter_, libff_liop::log2(this->num_constraints_));
}
...
{
    libff_liop::enter_block("Aurora SNARK prover");
    parameters.print();
    ...
    libff_liop::leave_block("Aurora SNARK prover");
    return transcript;
}
...
bool aurora_snark_verifier(const r1cs_constraint_system<FieldT> &constraint_system,
                           ...
                           const aurora_snark_parameters<FieldT, hash_type> &parameters)
{
    libff_liop::enter_block("Aurora SNARK verifier");
    parameters.print();

    bcs_verifier<FieldT, hash_type> IOP(parameters.bcs_params_, proof);

    libff_liop::enter_block("Aurora IOP protocol initialization and registration");
    aurora_iop<FieldT> full_protocol(IOP, constraint_system, parameters.iop_params_);
    full_protocol.register_interactions();
    IOP.seal_interaction_registrations();
    full_protocol.register_queries();
    IOP.seal_query_registrations();
    libff_liop::leave_block("Aurora IOP protocol initialization and registration");
    ...
    libff_liop::leave_block("Aurora SNARK verifier");
