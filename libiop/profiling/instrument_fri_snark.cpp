                    ...
                    std::size_t num_query_repetitions)
{
    for (std::size_t log_n = options.log_n_min; log_n <= options.log_n_max; ++log_n)
    {
        ...
        params.field_type_ = libff_liop::get_field_type<FieldT>(FieldT::zero());
        params.num_oracles_ = num_oracles;

        const FRI_snark_proof<FieldT, hash_type> proof = FRI_snark_prover<FieldT, hash_type>(params);
        printf("\n");
        libff_liop::print_indent(); printf("* Argument size in bytes (IOP): %zu\n", proof.IOP_size_in_bytes());
        libff_liop::print_indent(); printf("* Argument size in bytes (BCS): %zu\n", proof.BCS_size_in_bytes());
        libff_liop::print_indent(); printf("* Argument size in bytes (total): %zu\n", proof.size_in_bytes());

        printf("\nIf we were to remove the pruning of BCS merkle tree paths feature,\n"
               "the argument would have the following sizes:\n");
        libff_liop::print_indent(); printf("* Argument size in bytes (BCS, no pruning): %zu\n", proof.BCS_size_in_bytes_without_pruning());
        libff_liop::print_indent(); printf("* Argument size in bytes (total, no pruning): %zu\n", proof.size_in_bytes_without_pruning());
        printf("\n");
    ...
    if (default_vals.is_multiplicative) {
        switch (default_vals.field_size) {
            case 181:
                ...
                instrument_FRI<libff_liop::edwards_Fr, binary_hash_digest>(default_vals, localization_parameter,
                                     num_interactive_repetitions, num_query_repetitions,
                                     num_localization_steps, num_oracles);
                break;
            case 256:
                libff_liop::alt_bn128_pp::init_public_params();
                instrument_FRI<libff_liop::alt_bn128_Fr, binary_hash_digest>(default_vals, localization_parameter,
                                     num_interactive_repetitions, num_query_repetitions,
                                     num_localization_steps, num_oracles);
                break;
            default:
        ...
        {
            case 64:
                instrument_FRI<libff_liop::gf64, binary_hash_digest>(default_vals, localization_parameter,
                                     num_interactive_repetitions, num_query_repetitions,
                                     num_localization_steps, num_oracles);
                break;
            case 128:
                instrument_FRI<libff_liop::gf128, binary_hash_digest>(default_vals, localization_parameter,
                                     num_interactive_repetitions, num_query_repetitions,
                                     num_localization_steps, num_oracles);
                break;
            case 192:
                instrument_FRI<libff_liop::gf192, binary_hash_digest>(default_vals, localization_parameter,
                                     num_interactive_repetitions, num_query_repetitions,
                                     num_localization_steps, num_oracles);
                break;
            case 256:
                instrument_FRI<libff_liop::gf256, binary_hash_digest>(default_vals, localization_parameter,
                                     num_interactive_repetitions, num_query_repetitions,
                                     num_localization_steps, num_oracles);
                break;
            default:
