...
{
    if (libff_liop::is_multiplicative<FieldT>::value) {
...
void field_subset<FieldT>::construct_internal(const std::size_t num_elements,
                                              const typename libff_liop::enable_if<libff_liop::is_multiplicative<FieldT>::value, FieldT>::type coset_shift)
{
...
template<typename FieldT>
void field_subset<FieldT>::construct_internal(const std::size_t num_elements,
                                              const typename libff_liop::enable_if<libff_liop::is_additive<FieldT>::value, FieldT>::type coset_shift)
{
    assert(libff_liop::is_power_of_2(num_elements));
    const std::size_t dimension = libff_liop::log2(num_elements);
...
field_subset<FieldT> field_subset<FieldT>::get_subset_of_order(const std::size_t order) const
{
    switch (this->type_)
    {
        case affine_subspace_type:
        {
            const std::size_t subset_dim = libff_liop::log2(order);
