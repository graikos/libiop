...
void lagrange_cache<FieldT>::construct_internal(const multiplicative_coset<FieldT> &domain)
{
    ...
    FieldT v_inv_i = FieldT(m) * libff_liop::power(domain.shift(), m - 1);
    ...
    const FieldT &interpolation_point)
{
    ...
    result.reserve(m);
    const FieldT x_to_m = libff_liop::power(interpolation_point, m);
                                          ...
                                          const FieldT &interpolation_point)
{
    ...
    const FieldT Z_x = libff_liop::power(interpolation_point, m) - libff_liop::power(h, m);
    FieldT l = Z_x.inverse() * FieldT(m) * libff_liop::power(h, m - 1);
    FieldT r = h;
    for (size_t i = 0; i < m; ++i)
    {
