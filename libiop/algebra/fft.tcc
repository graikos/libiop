...
{
    libff_liop::enter_block("Call to additive_FFT_wrapper");
    libff_liop::print_indent(); printf("* Vector size: %zu\n", v.size());
    libff_liop::print_indent(); printf("* Subspace size: %zu\n", H.num_elements());
    const std::vector<FieldT> result = additive_FFT(v, H);
    libff_liop::leave_block("Call to additive_FFT_wrapper");
    return result;
}
...
std::vector<FieldT> additive_IFFT_wrapper(const std::vector<FieldT> &v,
                                          const affine_subspace<FieldT> &H)
{
    libff_liop::enter_block("Call to additive_IFFT_wrapper");
    libff_liop::print_indent(); printf("* Vector size: %zu\n", v.size());
    libff_liop::print_indent(); printf("* Subspace size: %zu\n", H.num_elements());
    const std::vector<FieldT> result = additive_IFFT(v, H);
    libff_liop::leave_block("Call to additive_IFFT_wrapper");
    return result;
}
                                                    ...
                                                    const FieldT &shift)
{
    ...
    const size_t n = coset.num_elements(), logn = libff_liop::log2(n);
    ...
    const size_t poly_dimension = libff_liop::log2(poly_coeffs.size());
    ...
    {
        const size_t rk = libff_liop::bitreverse(k, logn);
        if (k < rk)
        {
            std::swap(a[k], a[rk]);
        }
...
std::vector<FieldT> multiplicative_FFT_internal(
    const std::vector<typename libff_liop::enable_if<libff_liop::is_multiplicative<FieldT>::value, FieldT>::type> &poly_coeffs,
...
template<typename FieldT>
std::vector<FieldT> multiplicative_FFT_internal(
    const std::vector<typename libff_liop::enable_if<libff_liop::is_additive<FieldT>::value, FieldT>::type> &poly_coeffs,
    const multiplicative_subgroup_base<FieldT> &domain, const FieldT shift)
{
...
std::vector<FieldT> multiplicative_IFFT_internal(
    const std::vector<typename libff_liop::enable_if<libff_liop::is_multiplicative<FieldT>::value, FieldT>::type> &evals,
    const multiplicative_subgroup_base<FieldT> &domain, const FieldT shift)
{
...
template<typename FieldT>
std::vector<FieldT> multiplicative_IFFT_internal(
    const std::vector<typename libff_liop::enable_if<libff_liop::is_additive<FieldT>::value, FieldT>::type> &evals,
    const multiplicative_subgroup_base<FieldT> &domain, const FieldT shift)
{
    throw std::invalid_argument("attempting to perform multiplicative IFFT with non-multiplicative field type");
}
...
{
    libff_liop::enter_block("Call to multiplicative_FFT_wrapper");
    libff_liop::print_indent(); printf("* Vector size: %zu\n", v.size());
    libff_liop::print_indent(); printf("* Subgroup size: %zu\n", H.num_elements());
    const std::vector<FieldT> result = multiplicative_FFT(v, H);
    libff_liop::leave_block("Call to multiplicative_FFT_wrapper");
    return result;
}
...
std::vector<FieldT> multiplicative_IFFT_wrapper(const std::vector<FieldT> &v,
                                                const multiplicative_coset<FieldT> &H)
{
    libff_liop::enter_block("Call to multiplicative_IFFT_wrapper");
    libff_liop::print_indent(); printf("* Vector size: %zu\n", v.size());
    libff_liop::print_indent(); printf("* Coset size: %zu\n", H.num_elements());
    if (v.size() == 1)
    {
        libff_liop::leave_block("Call to multiplicative_IFFT_wrapper");
        return {v[0]};
    }
...
template<typename FieldT>
std::vector<FieldT> FFT_over_field_subset(const std::vector<typename libff_liop::enable_if<libff_liop::is_multiplicative<FieldT>::value, FieldT>::type> coeffs,
...
template<typename FieldT>
std::vector<FieldT> FFT_over_field_subset(const std::vector<typename libff_liop::enable_if<libff_liop::is_additive<FieldT>::value, FieldT>::type> coeffs,
                                          field_subset<FieldT> domain)
{
...
template<typename FieldT>
std::vector<FieldT> IFFT_over_field_subset(const std::vector<typename libff_liop::enable_if<libff_liop::is_multiplicative<FieldT>::value, FieldT>::type> evals,
                                           field_subset<FieldT> domain)
{
...
template<typename FieldT>
std::vector<FieldT> IFFT_over_field_subset(const std::vector<typename libff_liop::enable_if<libff_liop::is_additive<FieldT>::value, FieldT>::type> evals,
                                           field_subset<FieldT> domain)
{
    ...
    field_subset<FieldT> domain)
{
     ...
     */
    const size_t closest_power_of_two = libff_liop::round_to_next_power_of_2(degree);
