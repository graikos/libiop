...
TEST(PolynomialTest, Initialization) {
    const linearized_polynomial<libff_liop::gf64> lin_poly =
        linearized_polynomial<libff_liop::gf64>::random_linearized_polynomial(4);
}
                          ...
                          const bool vanishes)
{
    ...
    for (std::size_t i = 0; i < S.num_elements(); ++i)
    {
        ...
        const FieldT high_term = highest_coeff * libff_liop::power<FieldT>(S_elements[i], (1ull<<dim));
