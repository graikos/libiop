    ...
    bool exp_pass)
{
    ...
    {
        const size_t cur_size = libff_liop::round_to_next_power_of_2(
    ...
    oracle_handle_ptr fz_handle =
        std::make_shared<oracle_handle>(IOP.register_oracle("fz", codeword_domain_handle, fz_degree, make_zk));
    std::vector<oracle_handle_ptr> Mz_handles;
    for (std::size_t i = 0; i < matrices.size(); i++) {
        Mz_handles.emplace_back(
            std::make_shared<oracle_handle>(
                IOP.register_oracle("Mz", codeword_domain_handle, Mz_degree, make_zk)
            ));
    }
