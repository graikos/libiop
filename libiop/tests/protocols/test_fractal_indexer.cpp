...
void run_random_indexer_test(std::size_t domain_dim) {
    ...
    const size_t indexing_domain_dim = libff_liop::log2(M.num_nonzero_entries());
...
TEST(AdditiveTests, IndexerTest) {
    run_all_indexer_tests<libff_liop::gf64>();
    run_all_indexer_tests<libff_liop::gf128>();
    run_all_indexer_tests<libff_liop::gf192>();
    run_all_indexer_tests<libff_liop::gf256>();
}

TEST(MultiplicativeTests, IndexerTest) {
    libff_liop::edwards_pp::init_public_params();
    libff_liop::alt_bn128_pp::init_public_params();
    run_all_indexer_tests<libff_liop::edwards_Fr>();
    run_all_indexer_tests<libff_liop::alt_bn128_Fr>();
}
